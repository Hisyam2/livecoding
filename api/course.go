...
func (cr *courseAPI) DeleteCourse(c *gin.Context) {
	...
	if courseID == "" {
		c.JSON(http.StatusBadRequest, model.ErrorResponse{Error: "Course ID is required"})
		return
	}
	...
	c.JSON(http.StatusOK, model.SuccessResponse{Message: "Course deleted successfully"})
}
